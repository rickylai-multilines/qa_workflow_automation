...
def main():
    parser = argparse.ArgumentParser(description="Export TERMS rows to JSON.")
    ...
    parser.add_argument('--order-by', default='code')
    parser.add_argument('--output', default=os.path.join(os.getcwd(), 'foxpro_terms_export.json'))
    ...
    if args.all:
        query = (
            ...
            f"FROM TERMS{where_clause} ORDER BY {args.order_by}"
        )
    else:
        query = (
